...
async function loadProductions() {
  const data = await apiFetch('/productions?limit=50');
  if (!data) return;
...
function initLedger() {
  ...
  if (df && !df.value) df.value = `${y}-${mo}-01`;
...
window.goApp = function(page) {
  _origGoApp(page);
  if (page === 'production')        { loadProductions(); addProductionButton(); loadDeviceProdToday(); }
